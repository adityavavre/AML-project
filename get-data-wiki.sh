...
if [ ! -f $WIKI_PATH/txt/$lg.all ]; then
  python -m wikiextractor.WikiExtractor $WIKI_PATH/bz2/$WIKI_DUMP_NAME --processes 32 -q -o - \
  | sed "/^\s*\$/d" \
  | grep -v "^<doc id=" \
  | grep -v "</doc>\$" \
  | $TOKENIZE $lg \
...
split_data() {
    ...
    NTRAIN=$((NLINES - 10000));
    NVAL=$((NTRAIN + 5000));
    shuf $1 | head -n $NTRAIN             > $2;
    shuf $1 | head -n $NVAL | tail -n 5000  > $3;
    shuf $1 | tail -n 5000                > $4;
}
