...
class SentenceEmbedder(object):
    ...
    def get_parameters(self, layer_range):
        ...
        for l in range(max(i - 1, 0), j):
            if self.model.n_langs > 1 and self.model.lang_specific_ATTN:
                for lang in self.model.lang2id:
                    parameters += self.model.attentions[lang][l].parameters()
            else:
                parameters += self.model.attentions[l].parameters()
            if self.model.n_langs > 1 and self.model.lang_specific_LN:
                for lang in self.model.lang2id:
                    parameters += self.model.layer_norm1[lang][l].parameters()
            else:
                parameters += self.model.layer_norm1[l].parameters()
            if self.model.n_langs > 1 and self.model.lang_specific_FFN:
                for lang in self.model.lang2id:
                    parameters += self.model.ffns[lang][l].parameters()
            else:
                parameters += self.model.ffns[l].parameters()
            if self.model.n_langs > 1 and self.model.lang_specific_LN:
                for lang in self.model.lang2id:
                    parameters += self.model.layer_norm2[lang][l].parameters()
            else:
