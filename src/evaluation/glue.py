...
class GLUE:
    ...
    def eval(self, splt):
        ...
        if has_labels:
            ...
            if self.is_classif:
                ...
                scores['%s_f1_micro' % prefix] = f1_score(gold, pred, labels=list(range(params.out_features)),
                                                    average='binary' if params.out_features == 2 else 'micro')
                scores['%s_f1_macro' % prefix] = f1_score(gold, pred, labels=list(range(params.out_features)),
                                                    average='binary' if params.out_features == 2 else 'macro')
                scores['%s_f1_weighted' % prefix] = f1_score(gold, pred, labels=list(range(params.out_features)),
                                                    average='binary' if params.out_features == 2 else 'weighted')
    ...
    def load_data(self, task):
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            # load labels
            if splt in ('train', 'valid', 'test') or task in ['MRPC']:
